...
st.write("UPLOAD DATA FOR PREDICTIONS (csv format)")

upload = st.file_uploader(" ",type=["csv"])

if upload is not None:
    irrigation = pd.read_csv(upload)
    st.write("Data Preview:", irrigation.head())

    if st.button("Run Predictions"):
        prediction_encoded = model.predict(irrigation)

        predictions = le.inverse_transform((prediction_encoded))

        irrigation["predictions"] = predictions

        st.write("Predictions Complete", irrigation.sample(200))

        def download(irrigation):
            return irrigation.to_csv(index=False).encode("utf-8")
        
        predictions = download(irrigation)
...
if upload is None:
    ...
    with st.form("Prediction Form"):
        ...
        Rainfall_mm = st.number_input("Rainfall(mm)", 0.0, 2000.0, 100.0)
